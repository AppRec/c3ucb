...
the feedback is generated using click probability=ctr
...
def main(lamb, R):
    ...
    for r in xrange(train_rounds):
        ...
        for ii in range(K):  # count num of apps recommended
            if expl[action[ii]] == 0:
                expl[action[ii]] = 1
        # print expl

        reward = match_app.genCtr(action) # generate the action's reward based on apps' ctr
